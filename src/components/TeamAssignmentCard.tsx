...
interface TeamAssignmentCardProps {
    ...
    onRequestMove: (player: Player, teamId: string) => void;
}

export const TeamAssignmentCard = memo(({ team, index, onUpdateTeam, onRequestMove }: TeamAssignmentCardProps) => {
    return (
        <motion.div
            ...
            <ul className="space-y-2 relative z-10">
                {team.players.map(player => (
                    <li key={player.id} className="text-gray-300 flex items-center justify-between gap-2 group/player p-1 rounded hover:bg-white/5 transition-colors">
                        ...
                        <button
                            onClick={() => onRequestMove(player, team.id)}
